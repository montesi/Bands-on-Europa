function [zout,tout,fout,kout,hout,dn,r]=ConductionSolveRift(dz,tb,fb,vb,kfun,hfun,ns,di);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
while goon;
    ...
    z=0:nz-1;
    % get new thickness
    dnew=interp1(t,z,tb-1)*dz;
    % decide if go on or not
    if dnew>di; %there's a problem
        goon=0;
    elseif abs(dnew-dn)<dz; % almost no change
        goon=0;
    elseif dnew<dz; % almost no crust left
        goon=0;
    else 
        dn=dnew;
    end
...
r=integral(@(x)interp1(z*dz,t,x)-tb,0,dn);

zout=linspace(0,dn,ns);
